...
namespace area {
    ...
    template<typename T>
    std::ostream &operator<<(std::ostream &out, const Area2D<T> &area) {
        out << "[" << nc::NumberConverter<T>::convert(area.point1.x)
            << ", " << nc::NumberConverter<T>::convert(area.point2.x)
            << "]x[" << nc::NumberConverter<T>::convert(area.point1.y)
            << ", " << nc::NumberConverter<T>::convert(area.point2.y) << "]";
        return out;
    }
}