...
namespace drawer{
    ...
    void TriangulationDrawingConverter::convert_lines() {
        ...
        for(const auto& edge:edges){
            lines.push_back({sf::Vertex(sf::Vector2f(norm.normalize_x(edge->start->x), norm.normalize_y(edge->start->y)), DrawingColors::LINE),
                             sf::Vertex(sf::Vector2f(norm.normalize_x(edge->end->x), norm.normalize_y(edge->end->y)), DrawingColors::LINE)});
        }
    }
